      ...
      <div className="h-screen bg-[#0F0F0F] text-white font-sans flex flex-col overflow-hidden select-none relative">
         ...
         <div className="absolute top-16 left-0 right-0 z-10 pt-4 px-4 pb-2 bg-gradient-to-b from-[#0F0F0F] to-transparent">
          ...
          <div className="flex-grow min-h-0 relative flex flex-col items-center justify-center px-4 md:px-8 mt-24 mb-4">
            ...
            <div className="w-full max-w-4xl h-full max-h-[400px] bg-[#1A1A1A] rounded-[40px] border-b-8 border-r-8 border-white/10 flex flex-col md:flex-row items-center justify-center shadow-[0_30px_60px_rgba(0,0,0,0.8)] relative overflow-visible" style={{ transformStyle: 'preserve-3d', perspective: '1200px', transform: 'rotateX(5deg)' }}>
              <div className="absolute inset-0 opacity-5 pointer-events-none rounded-[40px] overflow-hidden" style={{ backgroundImage: 'radial-gradient(circle at center, white 1px, transparent 1px)', backgroundSize: '24px 24px' }}></div>
                   ...
                   {Array.from({ length: Math.min(gameState.players.length * 7, 28) }).map((_, i) => {
                     ...
                     return (
                         <motion.div
                           ...
                           className="absolute w-16 h-24 md:w-[90px] md:h-[135px] bg-[#2A2A2A] rounded-xl border-[4px] border-white/10 flex items-center justify-center shadow-2xl"
                         >
                            <div className="absolute inset-1.5 border-2 border-white/5 rounded-lg"></div>
                            <div className="text-xl md:text-3xl font-black italic text-white/20 rotate-45 drop-shadow-md">UNO</div>
                         </motion.div>
             ...
             <AnimatePresence>
               {funnyText && (
                 <motion.div
                   ...
                   style={{ top: '15%', transform: 'translateZ(50px)' }}
                 >
                   <div className="bg-yellow-400 text-black border-4 border-black font-black text-xl md:text-4xl italic px-8 py-4 rounded-3xl shadow-[5px_5px_0_0_#000] rotate-[-5deg]">
                     {funnyText.text}
                   </div>
                 </motion.div>
               )}
               {delayedText && (
                 <motion.div
                   ...
                   style={{ top: '25%', transform: 'translateZ(60px)' }}
                 >
                   <div className="bg-red-500 text-white border-4 border-white font-black text-lg md:text-3xl italic px-8 py-4 rounded-3xl shadow-[5px_5px_0_0_#fff] rotate-[3deg]">
                     {delayedText.text}
                   </div>
                 </motion.div>
               )}
             ...
             <div className="flex items-center gap-6 md:gap-14 z-10 scale-90 md:scale-100 mt-4 md:mt-0" style={{ transform: 'translateZ(10px)' }}>
               {/* Draw Pile Deck */}
               <div className="relative group cursor-pointer" onClick={() => { if(isMyTurn && gameState.specialAction.type === 'none') activeSocket.emit('draw_card') }}>
                 {/* Decorative background cards */}
                 <div className="w-24 h-36 md:w-32 md:h-48 bg-[#2A2A2A] rounded-xl border-4 border-white/10 absolute -top-1.5 -left-1.5 rotate-[-4deg] opacity-60"></div>
                 <div className="w-24 h-36 md:w-32 md:h-48 bg-[#2A2A2A] rounded-xl border-4 border-white/10 absolute top-1 left-1.5 rotate-[2deg] opacity-80"></div>
                 
                 <div className={`w-24 h-36 md:w-32 md:h-48 bg-[#2A2A2A] rounded-xl border-4 border-white/20 flex flex-col items-center justify-center shadow-2xl relative overflow-hidden transition-all duration-300 ${isMyTurn && gameState.specialAction.type === 'none' ? 'group-hover:-translate-y-2 group-hover:scale-105 group-hover:border-white/40 group-hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)]' : 'opacity-80'}`}>
                   <div className="absolute inset-2 border border-white/5 rounded-lg"></div>
                   <div className="text-3xl md:text-5xl font-black italic text-white/10 rotate-[35deg]">UNO</div>
                   <div className="absolute bottom-2 right-2 text-[10px] font-bold tracking-[0.2em] text-white/40">DRAW</div>
                 </div>
               </div>

               {/* Discard Pile */}
               <div className="relative w-24 h-36 md:w-[90px] md:h-[135px] shrink-0">
                 {discardPileUI.map((c, i) => (
                   <motion.div
                     ...
                     style={{ zIndex: i + 10 }}
                   >
                     {renderCard(c, undefined, undefined, "w-24 h-36 md:w-[90px] md:h-[135px]")}
                   </motion.div>
                 ))}
                 
                 {/* Active Color Indicator */}
                 {gameState.currentColor && (
                   <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute -right-8 md:-right-12 top-1/2 -translate-y-1/2 flex flex-col items-center gap-1.5 z-50 pointer-events-none">
                     <div className="text-[8px] md:text-[10px] uppercase font-black tracking-widest text-white/60 drop-shadow-md">COLOR</div>
                     <div className="w-8 h-8 md:w-10 md:h-10 rounded-full border-[3px] border-white shadow-xl" style={{ backgroundColor: colorHexMap[gameState.currentColor] }} />
                   </motion.div>
                 )}
               </div>
             </div>

             <div className="absolute bottom-4 flex items-center gap-4 px-6 py-2 bg-black/60 backdrop-blur rounded-full border border-white/10 z-10">
                <span className="text-[8px] md:text-[10px] uppercase tracking-[0.2em] text-white/60 font-bold hidden sm:block">Direction</span>
                <span className="text-[10px] font-black text-white px-2 py-0.5 rounded bg-white/10">{gameState.direction === 1 ? 'CLOCKWISE' : 'COUNTER'}</span>
                <div className={`text-green-400 font-bold ${gameState.direction === -1 ? 'scale-x-[-1]' : ''}`}>↻</div>
             </div>

             <AnimatePresence>
               {gameState.specialAction.type === 'choosing_color' && isMyTurn && (
                 <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="absolute z-30 inset-0 flex items-center justify-center bg-black/80 backdrop-blur-sm p-6 overflow-hidden">
                   <div className="bg-[#151515] border border-white/20 p-8 rounded-[40px] shadow-2xl shrink-0 flex flex-col items-center">
                     <h3 className="text-xl md:text-2xl font-black italic uppercase tracking-widest mb-8 text-center drop-shadow-md">Select Color</h3>
                     <div className="grid grid-cols-2 gap-4 md:gap-6">
                       {(['Red', 'Blue', 'Green', 'Yellow'] as const).map(c => (
                         <button
                           key={c}
                           onClick={() => activeSocket.emit('choose_color', c)}
                           className={`w-20 h-20 md:w-28 md:h-28 rounded-2xl border-[6px] shadow-2xl relative overflow-hidden group active:scale-95 transition-all
